...
| **Global** | `api::global.global` | 是 | 否 | 单例；系统级 SEO 默认值 + Organization 结构化数据 + 搜索引擎验证令牌 |
...
`src/bootstrap.ts` 每次启动自动为 Public 角色开放以下 12 个内容类型的 `find` + `findOne`：

`Site`、`Category`、`Product`、`Page`、`Global`、`Blog`、`News`、`SiteLayout`、`Keyword Cluster`、`Tag`、`FAQ`、`Author`。
